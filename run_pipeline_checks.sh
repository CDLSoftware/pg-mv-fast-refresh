...
echo "INFO: Connect to postgres database via PSQL session" >> $LOG_FILE
  psql --host=$HOSTNAME --port=$PORT --username=$PGUSERNAME --dbname=postgres -v MODULE_HOME=$MODULE_HOME -v SOURCEPASSWORD=$SOURCEPASSWORD -v SOURCEUSERNAME=$SOURCEUSERNAME << EOFC >> $LOG_FILE 2>&1
...
CREATE USER MAPPING IF NOT EXISTS for :SOURCEUSERNAME SERVER pgmv\$_instance OPTIONS (user :'MVUSERNAME', password :'MVPASSWORD');
